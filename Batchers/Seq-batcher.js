...
export async function main(ns) {
    ...
    let moneyThresh = ns.getServerMaxMoney(target) * 1;
                ...
                if (ns.hasRootAccess(server)) {
                    ...
                    if (threads > 0) {
                        if (ns.getServerSecurityLevel(target) > securityThresh) {
                            ...
                            threads = Math.min(threads, 0.1 / ns.hackAnalyze(target));
                            // let hackThreads = ns.hackAnalyzeThreads(target, ns.getServerMaxMoney(target) * 0.1);
                            ns.print("Hacked! And amount of threads per server: " + threads);    
                            ns.exec("/shared/hack.js", server, Math.floor(threads), target);
                        }
                    }
                }
            }
